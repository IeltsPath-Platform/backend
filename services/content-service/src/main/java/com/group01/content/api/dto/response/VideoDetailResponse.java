...
public record VideoDetailResponse(
    ...
    public static VideoDetailResponse from(VideoDetailResult result) {
        List<VideoSegmentResponse> segmentResponses = result.segments() != null
                ? result.segments().stream().map(VideoSegmentResponse::from).toList()
                : List.of();
