...
@Entity
@Table(name = "grading_jobs", uniqueConstraints = @UniqueConstraint(name = "uk_grading_idempotency", columnNames = {"user_id", "idempotency_key"}))
...
public class GradingJobJpaEntity {
    ...
    @Enumerated(EnumType.STRING) @Column(name = "grading_mode", nullable = false) private GradingMode gradingMode;
    @Enumerated(EnumType.STRING) @Column(nullable = false) private GradingJobStatus status;
    @Column(name = "point_cost_snapshot") private Integer pointCostSnapshot;
